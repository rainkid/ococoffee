...
-(id)initWithFrame:(CGRect)frame atTop:(BOOL)top  {
    ...
    if (self) {
        ...
        //_stateLabel.textAlignment = ;
        _stateLabel.textAlignment = UITableViewAutomaticDimension;
...
- (void)layouts {
    ...
    if (self.isAtTop) {
        ...
        dateFrame = CGRectMake(x,y,size.width,kPRLabelHeight);
        ...
        arrowFrame = CGRectMake(12*x, y-8, kPRArrowWidth, kPRArrowHeight);
...
- (void)dealloc {
    ...
    //[self removeObserver:self forKeyPath:@"contentOffsize"];
    //[_headerView release];
    //  [_footerView release];
    // [super dealloc];
}
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        // [self addObserver:self forKeyPath:@"contentOffsize" options:NSKeyValueObservingOptionNew context:nil];
...
- (void)tableViewDidScroll:(UIScrollView *)scrollView {
    ...
    //  NSLog(@"setHeaderViewState:%u",_headerView.state);
}
...
- (void)tableViewDidEndDragging:(UIScrollView *)scrollView {
    ...
    CGSize size = scrollView.frame.size;
    CGSize contentSize = scrollView.contentSize;
    NSLog(@"HeadView State :%u",_headerView.state);
    if (_headerView.state == kPRStateLoading || _footerView.state == kPRStateLoading) {
        return;
    }
...
- (void)flashMessage:(NSString *)msg{
    //Show message
    __block CGRect rect = CGRectMake(0, 0 - 20, self.bounds.size.width, 20);
